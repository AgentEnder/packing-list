...
export async function inviteUserToTrip(
  ...
  dispatch: (action: TripUsersActions) => void
...
export async function acceptTripInvitation(
  ...
  try {
    ...
    await loadTripMembers(state, tripId, dispatch);
...
export async function declineTripInvitation(
  ...
  try {
    ...
    const currentInvitations = state.tripUsers.pendingInvitations;
    dispatch(
      setPendingInvitations(
        currentInvitations.filter((inv) => inv.tripId !== tripId)
      )
    );
...
export async function loadTripMembers(
  ...
  try {
    ...
    const tripUsers: TripUser[] = (data || []).map((tu) => ({
      ...
      status: tu.status as TripUserStatus,
...
export async function populatePendingInvitations(): Promise<number> {
  try {
    const supabase = getSupabaseClient();
    const {
      data: { user },
    ...
    console.log(
      `📬 [INVITATIONS] Populated pending invitations for ${user.email}`
    );
